...
type
  ...
  public
    ...
    procedure RemoveSocket(Socket: TSplitSocket); overload;
    procedure RemoveSocket(Nr: Integer); overload;
    constructor Create;
...
begin
  if FLockedList = nil then
    raise Exception.Create('TMergeSocket.Count: List have to be locked first');
...
begin
  if FLockedList = nil then
    raise Exception.Create('TMergeSocket.GetSocket: List have to be locked first!');

  if (nr >= 0)and(nr < Count) then
    Result := TSplitSocket(FLockedList[nr])
...
###############################################################################}
procedure TMergeSocket.LockList;
begin
  FLockedList := FSocketList.LockList;
end;
...
procedure TMergeSocket.RemoveSocket(Nr: Integer);
var Socket: TSplitSocket;
begin
  if FLockedList = nil then
    raise Exception.Create('TMergeSocket.RemoveSocket: List have to be locked first');

  Socket := TSplitSocket(FLockedList[Nr]);
  Socket.OnDestroy := nil;
  Socket.OnNewPacket_ReadThread := nil;
  FLockedList.Delete(Nr);
...
procedure TMergeSocket.UnlockList;
begin
  FLockedList := NIL;
